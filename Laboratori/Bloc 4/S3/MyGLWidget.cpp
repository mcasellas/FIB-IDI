...
void MyGLWidget::initializeGL ()
{
    ...
    iniEscena();
    iniCamera();
    backfaceActivat = true;
    esFocusdePosicio = true;
    llumEnces = true;

    //posF = glm::vec3(1,1,1);
    posF = glm::vec3(0,0.5-1,0);
    posPatricio = glm::vec3(0.0,-1.0+0.15,0.0);
    posicioFocus();
    colorFocus();
    llumAmbient();
}
...
void MyGLWidget::posicioFocus() {
    glm::vec4 posi;
    posi = esFocusdePosicio ? (View * glm::vec4(posF,1.0)) : glm::vec4(posF,1.0);
...
void MyGLWidget::activarBackface() {
    backfaceActivat ? glEnable(GL_CULL_FACE) : glDisable(GL_CULL_FACE);
}

void MyGLWidget::modificarLlum() {
    colF = llumEnces ? glm::vec3(0.8, 0.8, 0.8): glm::vec3(0, 0, 0);;
}
...
void MyGLWidget::keyPressEvent(QKeyEvent* event)
{
    ...
    switch (event->key()) {
        ...
        case Qt::Key_J: { // ON_OFF Llum
            llumEnces = not llumEnces;
            colorFocus();
            break;
        }
        case Qt::Key_B: { // Backface culling
            backfaceActivat = not backfaceActivat;
            activarBackface();
            break;
        }
